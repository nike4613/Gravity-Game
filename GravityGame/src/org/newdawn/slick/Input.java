...
import net.mc42.games.SlickInput;
...
public class Input {
	...
	public void considerDoubleClick(int button, int x, int y) {
		if (doubleClickTimeout == 0) {
			...
			SlickInput.addIncomingEvent(
					new Event(EventType.MOUSE_CLICK.setData(button))
					.setData(1)
	...
	public void poll(int width, int height) {
		...
		while (Keyboard.next()) {
			if (Keyboard.getEventKeyState()) {
				...
				SlickInput.addIncomingEvent(
						new Event(EventType.KEY_DOWN.setData(eventKey))
						.setData(keys[eventKey])
				);
				consumed = false;
				...
				SlickInput.addIncomingEvent(
						new Event(EventType.KEY_UP.setData(eventKey))
						.setData(keys[eventKey])
				);
				consumed = false;
				for (int i=0;i<keyListeners.size();i++) {
		...
		while (Mouse.next()) {
			if (Mouse.getEventButton() >= 0) {
				if (Mouse.getEventButtonState()) {
					...
					SlickInput.addIncomingEvent(
							new Event(EventType.MOUSE_DOWN.setData(Mouse.getEventButton()))
							.setPos(new Pair<Integer, Integer>(pressedX,pressedY))
					);
					...
					SlickInput.addIncomingEvent(
							new Event(EventType.MOUSE_UP.setData(Mouse.getEventButton()))
							.setPos(new Pair<Integer, Integer>(releasedX,releasedY))
					);
				...
				if (Mouse.isGrabbed() && displayActive) {
					if ((Mouse.getEventDX() != 0) || (Mouse.getEventDY() != 0)) {
						consumed = false;
						SlickInput.addIncomingEvent(
								new Event(EventType.MOUSE_MOVE)
								.setPos(new Pair<Integer, Integer>(Mouse.getEventDX(), -Mouse.getEventDY()))
						);
				...
				if (dwheel != 0) {
					consumed = false;
					SlickInput.addIncomingEvent(
						new Event(EventType.MOUSE_WHEEL.setData(dwheel))
					);
					for (int i=0;i<mouseListeners.size();i++) {
						MouseListener listener = (MouseListener) mouseListeners.get(i);
						if (listener.isAcceptingInput()) {
		...
		if (!displayActive || Mouse.isGrabbed()) {
			...
			if ((lastMouseX != getMouseX()) || (lastMouseY != getMouseY())) {
				...
				SlickInput.addIncomingEvent(
						new Event(EventType.MOUSE_MOVE)
						.setPos(new Pair<Integer, Integer>(getMouseX(), getMouseY()))
				);
